...
def true_vc1dim_wave(
    ...
    Parameters
    ----------
    b0 : the coefficient from y0 = b0 z + e0, z~U(0,1), e0~N(0,var0).
    b1 : the coefficient from y1 = b1 z + e1, z~U(0,1), e1~N(0,var1).
    var0 : optional. The default is 1.0.
    var1 : optional. The default is 1.0.
    ...
    var1 = float(var1)
    if var0 < 0 or var1 < 0:
        raise ValueError("var0 and var1 must be nonnegative.")

    return (b0 - b1)**2 / 3.0 + (np.sqrt(var0) - np.sqrt(var1))**2
...
def true_vc_square_wave(
    ...
    ----------
    ONLY FOR dy = dz = 1
    b0 : the coefficient from y0 = b0 z ** 2 + e0, z~U(0,1), e0~N(0,S1).
    b1 : the coefficient from y1 = b1 z ** 2 + e1, z~U(0,1), e1~N(0,S2).
    S0 : optional. The default is identity.
    S1 : optional. The default is identity.
    ...
    Ez4 = 1.0 / 5.0  # E[Z^4] for Z ~ Unif(0,1)

    mean_term = (b0 - b1) ** 2 * Ez4
    var_term = (np.sqrt(S0) - np.sqrt(S1)) ** 2
...
def true_vc_prod1dim_wave(
    ...
    Parameters
    ----------
    b0, k0 : the coefficient from y0 = (b0 z + k0) * e0, z~U(0,1), e0~N(0,1).
    b1, k1 : the coefficient from y1 = (b1 z + k1) * e1, z~U(0,1), e0~N(0,1).
    SIM: Monte Carlo repetitions.
    ...
    ''' 
    db = float(b0) - float(b1)
    dk = float(k0) - float(k1)
    return (db * db) / 3.0 + (db * dk) + (dk * dk)
