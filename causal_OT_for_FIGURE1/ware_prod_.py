...
import numpy as np
from utilities import generate_data_prod_wave, treatment, cot_estimator
from truevalue import true_vc_prod1dim_wave
...
tvc = true_vc_prod1dim_wave(b0.item(), b1.item(), k0.item(), k1.item())
print("True Vc value:", tvc)
...
def main_cp(N, tvc, SIMSIZE, gap=True):
    ...
    for k in range(SIMSIZE):
        # generate raw data
        raw_data = generate_data_prod_wave(N, b0, b1, k0, k1)
        ...
        print(cot, LL_brd, LL_knn)
        print(f'{N}: {k+1}/{SIMSIZE} complete.')
...
# plt.grid(alpha=0.3)
# plt.tight_layout()
plt.savefig("wave_error_prod_error.pdf")
plt.show()
