...
import numpy as np
from wavelet import SmoothConditionalWasserstein, closed_form_cw_gaussian_dz2, closed_form_cw_gaussian_dz2_quad
...
def generate_data(dy, dz, n, m, rng):
    ...
    if dy == 2 and dz == 2:
        a, b, Sigma0, Sigma1 = d2_generate_data(dy, dz, n, m, rng)

    def mu0(z):
        # z: (N,2)
        return a * (z - 0.5)**2 + 0.3

    def mu1(z):
        return b * (z - 0.5)**2 + 0.3
...
def iteration(dy, dz, N, rng, etalist, pp=False):
    ...
    cw_closed = closed_form_cw_gaussian_dz2_quad(
...
def main(Nlist, etalist, dy, dz, seed, ITER=300):
...
def plot_results(Nlist, etalist, dy, dz, seed, res, ITER):
    ...
    plt.savefig(f"fig_2_dy{dy}_dz{dz}_seed{seed}.pdf")
    # plt.show()
...
def plot_logresults(Nlist, etalist, dy, dz, seed, res, ITER):
    ...
    plt.savefig(f"fig_2_log_dy{dy}_dz{dz}_seed{seed}.pdf")
    # plt.show()

if __name__ == "__main__":
    x = np.logspace(np.log10(1200), np.log10(3000), num=6)
