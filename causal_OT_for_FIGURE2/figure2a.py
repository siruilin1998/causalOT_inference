...
def generate_data(dy, dz, n, m, rng):
    ...
    if dy == 2 and dz == 2:
        base, slope, a, B, Sigma0, Sigma1 = d2_generate_data(dy, dz, n, m, rng)
...
def plot_results(Nlist, etalist, dy, dz, seed, res, ITER):
    ...
    plt.savefig(f"fig_1_dy{dy}_dz{dz}_seed{seed}.pdf")
    # plt.show()
...
def plot_logresults(Nlist, etalist, dy, dz, seed, res, ITER):
    ...
    plt.savefig(f"fig_1_log_dy{dy}_dz{dz}_seed{seed}.pdf")
    # plt.show()

if __name__ == "__main__":
    x = np.logspace(np.log10(800), np.log10(2500), num=6)
